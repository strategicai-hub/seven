"""
Rotas de observabilidade/logs. Prefixo: /seven/
"""
import json
import logging
...
async def logs_events(limit: int = 100):
    r = await get_redis()
    raw = await r.lrange("seven:logs", 0, limit - 1)
    events = []
    for item in raw:
        try:
...
<head>
<meta charset="UTF-8">
<title>Seven Academia — Painel</title>
<style>
...
<body>
<h1>Seven Academia — Execuções</h1>
...
async function refresh() {
  try {
    const res = await fetch('/seven/logs/events?limit=50');
